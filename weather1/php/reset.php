...
if(isset($_REQUEST["email"])) {
    ...
    if (strlen($email) < 1) {
        $problems = true;
        $reset_error = 'You must enter your email address.';
    }
    if (!preg_match('/@/', $email)) {
        $problems = true;
        $reset_error = 'You must enter a valid email address.';
    } else {
...
if( ! $problems){
    $sql = "SELECT id FROM user_weather 
        WHERE email = '".$email."' ";
    ...
    if(mysqli_num_rows($result) == 1){
        ...
        $sql = "INSERT INTO `weather_reset_log` (`id`, `user_id`, `reset_token`, `timestamp`) 
	 		VALUES (NULL, '".$user_id."', '".$token."', NOW())";
        $result = mysqli_query($link, $sql);
        if(mysqli_affected_rows($link) == 1){
            $reset_link = "https://in-info-web4.informatics.iupui.edu/~brownrob/N413/weather1/reset.php?token=".$token;
            $to = $_REQUEST["email"];
...
'.$reset_link.'
            ...
            $headers = 'From: brownrob@in-info-web4.informatics.iupui.edu' . "\r\n" .
                'Reply-To:brownrob@in-info-web4.informatics.iupui.edu' . "\r\n" .
                'X-Mailer: PHP/' . phpversion();

            mail($to, $subject, $message, $headers);
        }else{
            $reset_error = "There was a problem with the database.  Your password cannot be reset.";
        }

    }else{
        $reset_error = "The e-mail address you entered was not found in the database.<br/>
...
if ($user_id > 0){
    ...
    $data["status"] = 'failed';
    if($reset_error > ""){ $data["reset_error"] = $reset_error; }
    echo json_encode($data);
}
